...
class storage_hdf5(storage_class):
    def __init__(self, config):
        ...
        try:
            os.remove(f'{config["annotation_file"].rstrip(".txt")}.h5')
        except:
            pass
        self.dataset = h5py.File(f'{config["annotation_file"].rstrip(".txt")}.h5', "w")
    ...
    def lock(self):
        ...
        self.dataset = h5py.File(
            f'{self.config["annotation_file"].rstrip(".txt")}.h5', "r"
        )
        self.__locked = True
    ...
    def __getitem__(self, idx):
        ...
        return Image.fromarray(self.dataset["input"][idx]), Image.fromarray(
            self.dataset["target"][idx]
        )
...
class storage_raw(storage_class):
    def __init__(self, config):
        ...
        try:
            shutil.rmtree(config["annotation_file"].rstrip(".txt"))
        except:
            pass
...
def colorize(data):
    ...
    return np.array([color_map[pixel] for pixel in data.ravel()]).reshape(
        *data.shape, 3
    )
